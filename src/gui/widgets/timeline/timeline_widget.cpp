...
void TimelineWidget::set_document(model::Document* document)
{
    ...
    if ( document )
    {
        ...
        connect(document->main_composition(), &model::AnimationContainer::last_frame_changed, this, &TimelineWidget::update_timeline_end);
        update_timeline_end(document->main_composition()->last_frame.get());
        update_timeline_start(document->main_composition()->first_frame.get());
